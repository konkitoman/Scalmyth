...
public abstract class MoveControlMixin {
    ...
    @Shadow
    protected abstract boolean isWalkable(float relativeX, float relativeZ);
    ...
    @Shadow
    protected abstract float rotlerp(float sourceAngle, float targetAngle, float maximumChange);
    ...
    public void tick() {
        if (this.operation == MoveControl.Operation.STRAFE) {
            ...
            double W = this.mob.getBbWidth() / 2;
            if (Math.abs(d1) > W) {
                d0 *= W * 2;
            } else if (Math.abs(d0) > W) {
                d1 *= W * 2;
            }
